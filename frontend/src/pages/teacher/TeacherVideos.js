      ...
      try {
        ...
        for (const course of courses) {
          ...
          const videosWithCourse = videosResponse.data.map(video => ({
            ...
            courseId: course._id,
            // Attempt to propagate sectionId if the API already returns it per video or per course (common patterns: video.sectionId, video.section, course.sectionId)
            sectionId: video.sectionId || video.section || course.sectionId || course.section || undefined
          }));
                      ...
                      <Box sx={{ display: 'flex', gap: 1 }}>
                        ...
                        {video.sectionId && (
                          <GroupChatButton
                            size="small"
                            onClick={() => {
                              setChatContext({ courseId: video.courseId, sectionId: video.sectionId, courseTitle: video.courseName });
                              setChatOpen(true);
                            }}
                          />
                        )}
