  ...
  useEffect(() => {
    const fetchCourseAndVideos = async () => {
      try {
        ...
        // Capture course + any section reference for chat context
        setCourse(videosResponse.course);
        // Derive sectionId from possible fields
        const sectionId = videosResponse.course?.sectionId || videosResponse.course?.section || videosResponse.sectionId || videosResponse.section || null;
        if (sectionId && courseId) {
          setChatContext({ courseId, sectionId, courseTitle: videosResponse.course?.title || 'Course' });
        }
                ...
                <Paper sx={{ p: 2, mb: 3 }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 2 }}>
                    <Typography variant="h4" gutterBottom sx={{ mb: 0 }}>
                      {course ? course.title : 'Course Videos'}
                    </Typography>
