  ...
  useEffect(() => {
    const fetchCourseAndUnits = async () => {
      try {
        ...
        // Attempt to derive sectionId from course or first unit for chat context
        const sectionId = (unitsResponse[0]?.course?.sectionId) || (unitsResponse[0]?.course?.section) || unitsResponse[0]?.sectionId || unitsResponse[0]?.section || null;
        if (sectionId && courseId) {
          setChatContext({ courseId, sectionId, courseTitle: unitsResponse[0]?.course?.title || 'Course' });
        }
