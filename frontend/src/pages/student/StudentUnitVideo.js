  ...
  useEffect(() => {
    const fetchUnitAndVideos = async () => {
      try {
        ...
        setUnit(currentUnit);
        // Derive sectionId for chat context
        const sectionId = currentUnit.course?.sectionId || currentUnit.course?.section || currentUnit.sectionId || currentUnit.section || null;
        if (sectionId) {
          setChatContext({ courseId, sectionId, courseTitle: currentUnit.course?.title || 'Course' });
        }
