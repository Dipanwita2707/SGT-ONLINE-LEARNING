...
const DeanAnalytics = () => {
  ...
  const COLORS = ['#1976d2', '#2e7d32', '#ed6c02', '#9c27b0', '#26a69a', '#7b1fa2', '#d32f2f', '#ff5722'];
  ...
  const studentEngagementData = useMemo(() => {
    if (!data?.studentAnalytics) return [];
    return data.studentAnalytics.slice(0, 10).map(student => ({
      name: student.name,
      watchTime: Math.round(student.totalWatchTime / 3600), // Convert to hours
      courses: student.courseCount,
      progress: Math.round(student.avgProgress || 0)
    }));
  }, [data]);
