        ...
        for (let i = 0; i < rows.length; i++) {
          ...
          try {
            ...
            if (sectionRaw) {
              ...
              if (section) sectionId = section._id;
            }
            ...
            if (sectionId) {
              try {
                ...
                if (sectionDoc && !sectionDoc.students.includes(savedStudent._id)) {
                  sectionDoc.students.push(savedStudent._id);
                  await sectionDoc.save();
                }
              } catch (sectionError) {
                console.warn(`Failed to assign student to section ${sectionId}:`, sectionError.message);
              }
            }
            successCount++;
...
exports.createCourse = async (req, res) => {
  try {
    ...
    // Forum functionality removed - no longer creating default forum
    // const Discussion = require('../models/Discussion'); - REMOVED
    // const courseDiscussion = new Discussion({ ... }); - REMOVED
    // await courseDiscussion.save(); - REMOVED
    // console.log('Created default discussion forum...'); - REMOVED
