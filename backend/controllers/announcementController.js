    ...
    switch (senderRole) {
      ...
      case 'dean':
        ...
        if (targetAudience.isGlobal) {
          ...
          // Validate dean is announcing within their school for non-global announcements
          if (targetAudience.targetSchools && 
              !targetAudience.targetSchools.includes(context.school?._id?.toString())) {
            return res.status(403).json({ 
              message: 'Deans can only announce to their own school and departments (or make global announcements)' 
            });
          }

          // If targeting departments, ensure they belong to dean's school
          if (targetAudience.targetDepartments && targetAudience.targetDepartments.length > 0) {
