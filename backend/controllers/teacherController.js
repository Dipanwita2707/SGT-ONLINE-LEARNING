...
// Get forums assigned to the teacher (section-based)
exports.getTeacherForums = async (req, res) => {
  try {
    // Get unique courses from teacher's sections
  const sections = await Section.find({ $or: [{ teacher: req.user._id }, { teachers: req.user._id }] }).populate('courses', '_id title courseCode');
    const courseMap = new Map();
    sections.forEach(s => s.courses?.forEach(c => c && courseMap.set(c._id.toString(), c)));
...
// Get forums for a specific course (section-based auth)
exports.getCourseForums = async (req, res) => {
  try {
    ...
    // For teachers, verify they are assigned to a section with this course
  const teacherHasCourse = await Section.exists({ $or: [{ teacher: req.user._id }, { teachers: req.user._id }], courses: courseId });
    if (!teacherHasCourse) {
      return res.status(403).json({ message: 'Not authorized to access this course' });
    }
...
exports.getForumDiscussion = async (req, res) => {
  try {
    ...
    // Verify teacher is assigned to a section that contains this course
  const teacherHasCourse = await Section.exists({ $or: [{ teacher: req.user._id }, { teachers: req.user._id }], courses: discussion.course._id });
    if (!teacherHasCourse) {
...
exports.postForumReply = async (req, res) => {
  try {
    ...
    // Find the discussion
    const discussion = await Discussion.findById(forumId).populate('course', 'title');
    
    if (!discussion) {
      return res.status(404).json({ message: 'Forum not found' });
    }
    
    // Verify teacher is assigned to a section with this course
  const teacherHasCourse = await Section.exists({ $or: [{ teacher: req.user._id }, { teachers: req.user._id }], courses: discussion.course._id });
    if (!teacherHasCourse) {
...
exports.getTeacherAnalyticsOverview = async (req, res) => {
  try {
    ...
    // Get actual forum count for teacher's courses
    const Discussion = require('../models/Discussion');
    const forumCount = await Discussion.countDocuments({ 
      course: { $in: courseIds } 
    });
