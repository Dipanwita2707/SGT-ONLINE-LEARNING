...
exports.checkUnitQuizAvailability = async (req, res) => {
  ...
  const remainingAttempts = quizPassed ? 0 : Math.max(0, attemptLimit - attemptsTaken);
    const isLocked = !!(unitProgress.securityLock && unitProgress.securityLock.locked);
    const available = hasQuiz && allVideosWatched && !quizPassed && !isLocked && attemptsTaken < attemptLimit;
    ...
    res.json({
      ...
      quizCompleted,
      quizPassed,
      canTakeQuiz: hasQuiz && allVideosWatched && !quizPassed && !isLocked && attemptsTaken < attemptLimit,
      isLocked,
      lockInfo: isLocked ? {
...
exports.generateUnitQuiz = async (req, res) => {
  try {
    ...
    // Enforce attempt limit (3 attempts)
    const attemptsTaken = await QuizAttempt.countDocuments({
      student: studentId,
      unit: unitId,
      $or: [
...
exports.submitUnitQuiz = async (req, res) => {
  ...
  if (securityViolations.length > 0 || tabSwitchCount > 0) {
    ...
    if (securityViolations.length > 0 || tabSwitchCount > 3) {
      // Apply penalty for security violations (e.g., 10% reduction)
      securityPenalty = Math.min(20, securityViolations.length * 5 + (tabSwitchCount > 3 ? 10 : 0));
