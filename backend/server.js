...
try {
  ...
  console.log('🔐 HTTPS server configured with mkcert certificates');
} catch (error) {
  // Fallback to HTTP if certificates are not available
  server = http.createServer(app);
  console.log('⚠️  HTTPS certificates not found, using HTTP server');
}
...
server.listen(PORT, '0.0.0.0', async () => {
  ...
  console.log(`🚀 Server running on ${protocol}://0.0.0.0:${PORT} and accessible on your WiFi network.`);
  console.log(`🌐 Access at: ${protocol}://10.20.58.236:${PORT} or ${protocol}://localhost:${PORT}`);
});
